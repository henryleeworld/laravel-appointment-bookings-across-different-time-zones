...
class BookingNotificationsTest extends TestCase
{
    ...
    public function test_creating_a_booking_schedules_notifications(): void
    {
        ...
        $startTime = now('UTC')->addDay();
        $this->actingAs($user)
            ->post(route('booking.store'), [
                'start' => $startTime,
                'end' => now('UTC')->addDay()->addHours(2)
            ])
    ...
    public function test_scheduler_sends_the_notifications_out(): void
    {
        ...
        Notification::fake();
        Carbon::setTestNow(now('UTC')->addHours(6));
    ...
    public function test_tries_are_increasing_when_failed(): void
    {
        ...
        Artisan::call('send:scheduled-notifications');
        Notification::assertNothingSent();
        $this->assertEquals(config('app.notification.attempt'), $booking->scheduledNotifications()->where('scheduled_at', $twoHoursTimeToCheck)->first()->tries);
        $this->assertEquals(config('app.notification.attempt'), $booking->scheduledNotifications()->where('scheduled_at', $oneHourTimeToCheck)->first()->tries);
        $this->assertEquals(config('app.notification.attempt'), $booking->scheduledNotifications()->where('scheduled_at', $fiveMinTimeToCheck)->first()->tries);
        $this->assertEquals(config('app.notification.attempt'), $booking->scheduledNotifications()->where('scheduled_at', $startTimeToCheck)->first()->tries);
    }
    ...
    public function test_editing_booking_start_date_changes_scheduled_notifications(): void
    {
        ...
        $newStartTime = now('UTC')->addDays(2);
        $this->actingAs($user)
