...
class Booking extends Model
{
    ...
    public function createReminderNotifications(Booking $booking, CarbonImmutable $startTime): void
    {
        ...
        if (now(config('app.timezone'))->lessThan($twoHoursTime)) {
        ...
        if (now(config('app.timezone'))->lessThan($oneHourTime)) {
            $booking->user->scheduledNotifications()->create([
        ...
        if (now(config('app.timezone'))->lessThan($fiveMinutesTime)) {
            $booking->user->scheduledNotifications()->create([
        ...
        if (now(config('app.timezone'))->lessThan($startingTime)) {
            $booking->user->scheduledNotifications()->create([
